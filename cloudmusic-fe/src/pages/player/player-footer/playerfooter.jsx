...
const PlayerFooter = () => {
    ...
    const handleTimeUpdate = () => {
        ...
        if (isPlaying) {
            ...
            for (const i in currentLyrics) { // const i 的类型是String, 用Number包裹实现正确的索引和比较
                ...
                // -400是为了对齐时间
                if (Number(i) === currentLyrics.length - 1 || currentLyrics[Number(i) + 1]?.timeStamp - 400 > currentTime * 1000) {
                    index = i;
                    break;
                }
            }
    ...
    const handleChangeMusic = (isNext) => {
        setCurrentTime(0);
        setIsPlaying(false);
        var index = 0;
        if (playmode === 0 || playmode === 2) {// sequential / single     
            if (isNext) {
                ...
                index = currentIndex === 0 ? playList.length - 1 : (currentIndex - 1) % (playList.length)
            }
        } else {
    ...
    return (
        <PlayerFooterWrapper>
            ...
            <div className='content'>
                <PlayerFooterContentWrapper>
                    <div className='play-btns'>
                        <span className='prev' title='上一首(ctrl+<-)' onClick={() => handleChangeMusic(false)} />
                        <span className={`play-btn ${isPlaying ? 'play' : 'pause'}`} title='播放/暂停(P)' onClick={handlePlayPause} />
                        <span className='next' title='下一首(ctrl+->)' onClick={() => handleChangeMusic(true)} />
                    </div>
            ...
            <audio ref={audioRef} onTimeUpdate={handleTimeUpdate} onEnded={handleDtEnded} />
            {contextHolder}
        </PlayerFooterWrapper>
